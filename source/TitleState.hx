...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		//if(!FlxG.save.data.psykaEasterEgg || !easterEggEnabled) {
			gfDance = new FlxSprite(FlxG.width * 0.43, FlxG.height * 0.15);
		...
		buggoSpr = new FlxSprite(0, FlxG.height * 0.55).loadGraphic(Paths.image('buggo_logo'));
		add(buggoSpr);
		buggoSpr.visible = false;
		buggoSpr.setGraphicSize(Std.int(buggoSpr.width * 0.8));
		buggoSpr.updateHitbox();
		buggoSpr.screenCenter(X);
		buggoSpr.antialiasing = ClientPrefs.globalAntialiasing;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 3:
					createCoolText(['A Bunch Of Cool People']);
					addMoreText('Present');
				// credTextShit.text += '\npresent...';
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['Original Animation By']);
				case 7:
					addMoreText('J-Bug');
					buggoSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 8:
					deleteCoolText();
					buggoSpr.visible = false;
				...
				// credTextShit.screenCenter();
				case 9:
					createCoolText(['this build is still a demo']);
				// credTextShit.visible = true;
				case 11:
					addMoreText('stay tuned for the full week');
				// credTextShit.text += '\nlmao';
				case 12:
					deleteCoolText();
				// credTextShit.visible = false;
